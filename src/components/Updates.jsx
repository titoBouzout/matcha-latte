...
export default function Updates() {
  ...
  async function checkUpdates() {
    const r = await Command.create("exec-sh", ["-c", "checkupdates"]).execute();
  ...
  ready(async () => {
    try {
      setUpdates(await rateLimited("updates", 60 * 1000 * 30, checkUpdates));
    } catch (error) {
  ...
  return (
    <Badge
      ...
      value={updateCount}
      tooltip={<For each={updates}>{(data) => <Update data={data} />}</For>}
...
function Update({ data }) {
  ...
  const what =
    +cur.major > (+prev.major || 0)
      ...
      ? "patch"
      : +cur.release?.replace("r", "") > +prev.release?.replace("r", "")
      ? "release"
  ...
  return (
    <div class="centered" style={"gap:8px; justify-content: space-between;"}>
